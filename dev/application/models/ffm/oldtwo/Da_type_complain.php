...
class Da_type_complain extends Ffm_model {
	...
	function insert() {
		...
		$sql = "INSERT INTO type_complain (tp_id, tp_complain)
				VALUES(?, ?)";
		$this->db->query($sql, array($this->tp_id, $this->tp_complain));
	...
	function update() {
		...
		$sql = "UPDATE type_complain 
				SET	tp_complain=? 
				WHERE tp_id=?";	
		$this->db->query($sql, array($this->tp_complain, $this->tp_id));	
	}
	...
	function delete() {
		...
		$sql = "DELETE FROM type_complain
				WHERE tp_id=?";
		$this->db->query($sql, array($this->tp_id));
	}
	...
	function get_by_key($withSetAttributeValue=FALSE) {	
		$sql = "SELECT * 
				FROM type_complain 
				WHERE tp_id=?";
		$query = $this->db->query($sql, array($this->tp_id));
