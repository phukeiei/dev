...
class M_field_reservation extends Da_field_reservation {
	...
	function get_all_price(){
		$sql = "SELECT fr_cost,ff_name ,SUM(fr_cost) as sum_cost,COUNT(fr_first_name) as sum_pp
		FROM field_reservation
		LEFT JOIN football_field on fr_ff_id = ff_id
		where fr_status = 2
        GROUP BY ff_id" ;
	...
	//นพ หาคคนในช่วงเวลา
	function find_date($date1,$date2){
		$sql ="SELECT ff_name ,COUNT(fr_first_name)as c_pp ,SUM(fr_cost) as sum_cost
		FROM field_reservation
        LEFT JOIN football_field on ff_id= fr_ff_id
		WHERE fr_start_time  BETWEEN  2019-05-31 AND 2019-05-1
        GROUP BY ff_name";
