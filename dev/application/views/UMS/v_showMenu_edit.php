	...
	<div class="col-md-12">	
		<div class="panel panel-teal panel-collapsed" data-widget='{"draggable": "false"}'>
			...
			<?php foreach($edit as $show){
					...
					$MnID = $this->encryption->encrypt($show['MnID']);
					$ID = str_replace("/","_",$MnID);
					$ID = str_replace("+",":",$ID);
					$MnID = str_replace("=","~",$ID);
			?>
			<form class="form-horizontal row-border" id="validate-form" data-parsley-validate method="post" action="<?php echo base_url(); ?>index.php/UMS/showSystem/updateMenu/<?php echo $MnID."/".$MnStID;?>">	
				<div class="panel-body">
					<div class="form-group">
						<label class="col-sm-1 control-label">ชื่อเมนู(ท)</label>
							<div class="col-sm-5">
								<input type="text" name="MnNameT" placeholder="ชื่อเมนูภาษาไทย" required class="form-control" value="<?php echo $show['MnNameT'];?>"/>
							</div>
					</div>
					<div class="form-group">
						<label class="col-sm-1 control-label">ชื่อเมนู(E)</label>
							<div class="col-sm-5">
								<input type="text" name="MnNameE" placeholder="ชื่อย่อระบบภาษาไทย" class="form-control" value="<?php echo $show['MnNameE'];?>">
							</div>
					</div>
					<div class="form-group">
						<label class="col-sm-1 control-label">URL</label>
						<div class="col-sm-5">
							<input type="text" name="MnURL" placeholder="ชื่อย่อระบบภาษาไทย" class="form-control" value="<?php echo $show['MnURL']; ?>">
						</div>
					</div>
					<div class="form-group">
						<label class="col-sm-1 control-label">คำอธิบาย</label>
						<div class="col-sm-5">
							<textarea style="resize:none" placeholder="คำอธิบายระบบ" name="MnDesc" cols="50" rows="4" class="form-control" value="<?php echo $show['MnDesc'];?>"></textarea>
						</div>
					</div>
					<div class="form-group">
						<label class="col-sm-1 control-label">ไอคอนของระบบ</label>
						<div class="col-sm-5">
							<input type="text" name="MnIcon" placeholder="ไอคอนของระบบ" id="da-ex-dialog-modal" value="<?php echo $show['MnIcon']?>" data-target="#myModal" data-toggle="modal" required class="form-control" data-toggle="modal">
						</div>
					</div>
				</div>
