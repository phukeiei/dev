...
class C_complains extends  UMS_Controller{
    function __construct(){
        parent::__construct();  
    }
    
    function Export_ffm_complains_non_member(){
        $this->output_frontend("ffm/fontend/ffm_complains_non_member");
    }

    function Export_ffm_complains_member(){
        $this->load->model('ffm/fontend/M_complains','Mc');
        $data['qu_Mc'] = $this->Mc->get_tp()->result(); 
        $this->output_frontend('ffm/fontend/ffm_complains_member',$data); 
    }
