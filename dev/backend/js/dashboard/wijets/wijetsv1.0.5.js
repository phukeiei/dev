		...
		if(data!=''){
			...
			$.each(json_obj,function(index,val){
				...
				if(val.widget_style == 'tile-card'){
						html_txt += '<div class="col-md-'+val.pn_size+' metric-tooltip" data-toggle="tooltip" data-html="true" title="สภาฯ&nbsp<a href=\'#\' target=\'_blank\'>4</a> , <a href=\'#\' target=\'_blank\'>12</a>">'+
						...
						html_txt += '<div class="btn-group btn-right" role="group">'+
													'<button type="button" class="btn btn-default-alt btn-sm pull-right expand-panel" expand-path="'+val.is_expand.link+'" i_frame="'+i_frame+'" data-param="'+val.is_expand.param+'"><i class="fa fa-search tooltips" data-original-title="ดูทั้งหมด"></i></button>'+
				...
				if(val.widget_style == 'tiles-card'){
					html_txt += '<div class="col-md-'+val.pn_size+'">'+
									'<h3 style=" border-radius:20px; background-color:'+val.pn_hd_color+'; text-align:center;" >&nbsp;'+val.tiles_heading+'&nbsp;</h3>';
					...
					html_txt +=			'<button class="btn btn-midnightblue btn-sm expand-button-new wrap" expand-path="'+tile.is_expand.link+'" i_frame="'+i_frame+'" data-param="'+tile.is_expand.param+'"><i class="fa fa-search"></i></button>';
									}
				...
				html_txt += '<div class="col-md-'+val.pn_size+'">'+
								'<div class="panel panel-danger">'+
									'<div class="panel-heading" style="background-color:'+val.pn_hd_color+';">'+
										...
										var i_frame = 0;
										if(typeof val.is_expand.iframe != 'undefined'){ i_frame = 1; }
					html_txt +=			'<button class="btn btn-midnightblue btn-sm expand-button-new wrap" expand-path="'+val.is_expand.link+'" i_frame="'+i_frame+'" data-param="'+val.is_expand.param+'"><i class="fa fa-search"></i></button>';
									}
				html_txt +=			'</div>'+
					...
					if(val.chart_type == 'ch1' || val.chart_type == 'ch4'){
						...
						grp_data.push({'data' : {'series': val.chart_data,'chart_cate':val.chart_cate,'show_legend': val.show_legend,'xaxis_tile':val.xaxis_tile,'chart_type' : val.chart_type, 'chart_sub_type' : val.chart_sub_type , 'chart_unit': val.chart_unit , 'chart_share' : val.chart_share ,'tooltip':val.tooltip }});
					...
					}else if(val.chart_type == 'eqs_sp1' || val.chart_type == 'eqs_sp2'){
						grp_data.push({'data' : {'series': val.chart_data,'chart_type':val.chart_type,'chart_cate':val.chart_cate}});
					}else if(val.chart_type == 'hr_countNurse' || val.chart_type == 'acd_sp1' || val.chart_type == 'ma_sp1'){
						grp_data.push({'data' : {'series': val.chart_data, 'chart_type':val.chart_type}});
						...
						grp_data.push({'data' : {'series': val.chart_data, 'chart_type':val.chart_type, 'chart_cate':val.chart_cate, 'chart_table_link':val.chart_table_link}});
					}
				}
				if(val.widget_style == 'table-card'){
					if(val.table_type == '1'){
						...
						html_txt +=								'<td class="text-right">'+tbody_val.col3+'</td>'+
																	 		'<td class="text-right"><button class="btn btn-midnightblue btn-sm expand-button-new" i_frame="'+i_frame+'" data-param="'+tbody_val.is_expand.param+'" expand-path="'+tbody_val.is_expand.link+'"><i class="fa fa-search"></i></button></td>';
																		}
						html_txt +=								'</tr>';
																	});
						html_txt +=						'</tbody>'+
						...
						html_txt +=								'<td width="60%">'+
																				'<div class="progress m-n">'+
																					'<div class="progress-bar" style="width:'+row_val.progress_len+'%;"></div>'+
																				'</div>'+
																			'</td>';
																			} else {
						...
						html_txt +=									 			'<button class="btn btn-midnightblue btn-sm expand-button-new" i_frame="'+i_frame+'" data-param="'+row_val.is_expand.param+'" expand-path="'+row_val.is_expand.link+'"><i class="fa fa-search"></i></button>';
																	 		}
						html_txt +=									 		'</td>';
																		}
						html_txt +=								'</tr>';
					...
					if(val.table_type == 'rs_sp1'){
						...
						html_txt += ''+
						'<div class="col-md-'+val.pn_size+'" id="dwnxx2">'+
							'<div class="panel  panel panel-blue" data-widget=\'{"draggable": "false"}\'>'+
								'<div class="panel-heading"  style="background-color:'+val.pn_hd_color+';" >'+
					...
					if(val.table_type == 'basic-npad'){
						...
						if(typeof val.table_data.tr != 'undefined'){
							...
							if(typeof val.is_expand != 'undefined'){
								...
								link_trigger = 'class="expand-button-new"  i_frame="'+i_frame+'" ';
							}
							$.each(val.table_data.tr, function(trdex, trval){
								var td = '';
								var param_tr = '';
				...
				if(val.widget_style == 'great-card'){
					...
					if(typeof val.re_great != 'undefined'){
						regret += '<div class="col-sm-1">'
						regret += '<select id="greater-card'+index_the_great+'" path="'+val.re_great.path+'" class="re_the_great form-control" style="text-align:center; display:inline-block;font-size:16px; height:39px;" >';
					...
					if(typeof val.is_expand != 'undefined' && val.is_expand!=false){
						expand = '<button class="btn btn-midnightblue btn-sm expand-button-new wrap" expand-path="'+val.is_expand.link+'" i_frame="0" data-param="'+val.is_expand.param+'"><i class="fa fa-search"></i></button>'
					}

					html_txt += '<div class="col-md-'+val.pn_size+' oh-the-great-card">'+
									'<div class="panel panel-danger">'+
										'<div class="panel-heading" style="background-color:'+val.pn_hd_color+';">'+
											'<h2 style="color:'+val.pn_hd_text_color+';">'+val.pn_hd_text+'</h2>'+regret+'';
							html_txt +=	'</div>'+
	...
	this.registerAction( {
		...
		onInit: function () {
			...
			if(selectType){
		        ...
		        widget.append('<div class="panel-loading"><div class="panel-loader-circular"></div></div>');
		        $.post( selectType , { path:selectType , param:selectParam , curId:flag })
		        .done(function (data) {
	...
	var makePlot = function (widget,param,payload){
        ...
        if(payload.chart_type == 'ch1'){
			...
			if(widget.find('#'+param).length > 0){
			      ...
			      chart_y_title = "จำนวน";
			      var process_data = function(element){
			                ...
			                point: {
			                    events: {
			                        click: function () {
								        ...
								        $.post( path , { path:path , title:head, param:param, curId:flag })
								        .done(function (data) {
											$.when(draw_card(data)).done(function (arg1){
												widget.find('.panel-loading').remove();
		...
		$.post(the_path+'/index.php/dashboard/Data_management/load_config2', { type:'r',grp_name:page_id }, function(data, textStatus, xhr) {
			...
			try{
				...
				$.each(obj.result,function(index,val){
					console.log(widget_grp+'.'+val.config_widget_name+'.draggable', 'false');
	...
	this.Mymake = function (settings) {
		...
		$(document.body).on('click','.select_da_course', function(){
			...
			if(current_crs != $(this).val()){
				...
				widget.append('<div class="panel-loading"><div class="panel-loader-circular"></div></div>');
				$.post( path , { path:path , crs_id:$(this).val() , curId:flag })
		        .done(function (data) {
					//console.log(data);
					$.when(draw_card(data)).done(function (arg1){
						widget.find('.panel-loading').remove();
		...
		$(document.body).on('change','.re_the_great', function(){
			...
			$.post( path , { path:path , parameter:parameter , curId:flag })
		    .done(function (data) {
				$.when(draw_card(data)).done(function (arg1){
