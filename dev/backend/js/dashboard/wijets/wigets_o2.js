...
$.wijets = new function () {
			...
			$.each(json_obj,function(index,val){
				...
				html_txt +=			'</div>'+
					...
					if(val.chart_type == 'ch1'){
						grp_data.push({'data' : {'series': val.chart_data,'chart_type':val.chart_type , 'chart_unit': val.chart_unit}});
					}else if(val.chart_type == 'ch2'){
						grp_data.push({'data' : {'series': val.chart_data,'chart_cate':val.chart_cate,'chart_type' : val.chart_type, 'chart_sub_type' : val.chart_sub_type , 'chart_unit': val.chart_unit}});
	...
	var the_path = da_path[0]+'//'+da_path[2]+'/'+da_path[3]+'/'+da_path[4];
	...
	$("body").on("click" , '#saveButton' , function(){
		...
		console.log(the_path+'/dashboard/Data_management/save_config2');
		 $.post( the_path+'/dashboard/Data_management/save_config2', {  param1: JSON.stringify(arr), grp_name: page_id }, function(data, textStatus, xhr) {
	...
	var load_local = function(){
		...
		$.post(the_path+'/dashboard/Data_management/load_config2', { type:'r',grp_name:page_id }, function(data, textStatus, xhr) {
			...
			var parameter = $(this).data('param');
			 ...
			 if($(this).attr('i_frame') != 1){
				...
				var frame = '<form id="open_frame" action="'+file_name+'" method="post" target="framdal"><input type="hidden" name="param" value="'+parameter+'" /></form>';
					frame += '<iframe style="width:100%;height:440px;" name="framdal" class="framdal" frameBorder="0" src=""></iframe>';
