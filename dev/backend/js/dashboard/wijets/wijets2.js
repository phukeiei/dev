	...
	var draw_card = function(data){
		if(data!=''){
			...
			$.each(json_obj,function(index,val){
				...
				if(val.widget_style == 'holy-card'){
					
					function gen_holy_tab(tab_data){
						var holy_tab = '';
						var list_nav = '';
						var tab_content = '';
						...
						holy_tab += '<div class="panel panel-midnightblue" data-widget=\'{"draggable": "false"}\' style="visibility: visible;"> '+
			 				'<div class="panel-heading" style="background-color:#37474f">'+
								...
								'<div class="input-group-btn" align="right">'+
									'<button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown">มากที่สุด <i class="fa fa-angle-down"></i></button>'+
					...
					var cond_g = 'max';
					html_txt = gen_holy_tab(val.tab_data);
					...
					$("body").on('click', '.holy_sorting', function(){
						...
						if(cond_g != cond){
							...
							var new_tabs = gen_holy_tab(temp_massive_data);
							var place_belong = $(this).parents('.panel-body');
