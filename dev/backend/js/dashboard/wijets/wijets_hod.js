			...
			$.each(json_obj,function(index,val){
				...
				if(val.widget_style == 'tile-card'){
						...
						html_txt += '<div class="btn-group btn-right" role="group">'+
													'<button type="button" class="btn btn-default-alt btn-sm pull-right expand-panel" expand-path="'+val.is_expand.link+'" data-param="'+val.is_expand.param+'"><i class="fa fa-search tooltips" data-original-title="ดูทั้งหมด"></i></button>'+
													'<button type="button" class="btn btn-default-alt btn-sm pull-right book-icon" data-toggle="modal" data-seq="'+index_the_great+'" data-target="#kpi_modal"><i class="fa fa-book tooltips"></i></button>'+
												'</div>';
										}
						html_txt +=	'<div class="tile-icon" style="color: '+val.tile_icocolor+';"><i class="'+val.tile_icon+'"></i></div>'+
											'<div class="tile-heading" style="color: black;"><span>'+val.tile_heading+':'+index_the_great+'</span></div>'+
											'<div class="tile-body"><span>'+val.tile_body+'</span></div>'+
											'<div class="tile-footer"><span class="text-danger">'+val.tile_footer+'</span></div>'+
											'<input type="hidden" class="kpi_name_'+index_the_great+'" value="'+val.widget_kpi.name+'">';
						$.each(val.widget_kpi.list, function(key, value) {
							kpi_key.push(key);
							kpi_value.push(value);
						});
						html_txt +=	'<input type="hidden" class="kpi_key" value="'+kpi_key+'" />'+
												'<input type="hidden" class="kpi_value" value="'+kpi_value+'" />'+
					 							'</div>'+
									'</div>';
						kpi_key = [];
						kpi_value = [];
				...
				html_txt +=			'</div>'+
					...
					if(val.chart_type == 'ch1'){
						grp_data.push({'data' : {'series': val.chart_data,'chart_type':val.chart_type , 'chart_unit': val.chart_unit, 'tooltip':val.tooltip }});
					}else if(val.chart_type == 'ch2'){
						grp_data.push({'data' : {'series': val.chart_data,'chart_cate':val.chart_cate,'chart_type' : val.chart_type, 'chart_sub_type' : val.chart_sub_type , 'chart_unit': val.chart_unit , 'chart_share' : val.chart_share}});
					}else if(val.chart_type == 'ch3'){
						grp_data.push({'data' : {'series': val.chart_data,'chart_cate':val.chart_cate,'chart_type' : val.chart_type, 'chart_unit': val.chart_unit}});
				...
				if(val.widget_style == 'great-card'){
					...
					if(typeof val.re_great != 'undefined'){
						...
						$.each(val.re_great.details, function(ingret, vagret){
							//console.log(54584545454545);
							regret += '<option value="'+vagret.param+'">'+vagret.name+'</option>';
						});
						regret += '</select>';
						regret += '</div>';
					}
...
console.log(controls);
	...
	var makePlot = function (widget,param,payload){
        ...
        if(payload.chart_type == 'ch1'){
        	...
        	var toolPoint = '<span style="font-size: 20px" align="right">{point.y} '+unit+'</span>';
			                ...
			                point: {
			                    events: {
			                        click: function () {
								        ...
								        $.post( path , { title:head, param:param })
								        .done(function (data) {
											$.when(draw_card(data)).done(function (arg1){
												widget.find('.panel-loading').remove();
			...
			var parameter = $(this).data('param');
			 ...
			 if($(this).attr('i_frame') != 1){
				...
				var frame = '<form id="open_frame" action="'+file_name+'" method="post" target="framdal"><input type="hidden" name="param" value="'+parameter+'" /></form>';
					...
					$('.modal-footer').append('<button style="float:left" type="button" class="btn btn-midnightblue-alt" id="frame_back">กลับ</button>');
			}
