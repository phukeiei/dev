	...
	d.extend(L.prototype, {
		...
		_attachments : function (a, b) {
			...
			if (c == "button" || c == "both") {
				...
				var cc;
				cc = d(this._get(b, "buttonImageOnly") ? d("<img/>").addClass(this._triggerClass).attr({
							src : f,
							alt : c,
							title : c
		...
		_selectDate : function (a, b) {
			...
			if (e)
				e.apply(c.input ? c.input[0] : null, [b, c]);
			else
		...
		parseDate : function (a, b, c, e) {
			...
			for (var h = (c ? c.dayNamesShort : null) || this._defaults.dayNamesShort, i = (c ? c.dayNames : null) || this._defaults.dayNames, g = (c ? c.monthNamesShort : null) || this._defaults.monthNamesShort, m = (c ? c.monthNames : null) || this._defaults.monthNames, q = c = -1, k = -1, l = -1, o = false, n = function (p) {
				...
				for (w = 0; w < p.length; w++)
					if (b.substr(x, p[w].length).toLowerCase() == p[w].toLowerCase()) {
						x += p[w].length;
						return w + 1
					}
				throw "Unknown name at position " + x;
			}, u = function () {
				if (b.charAt(x) != a.charAt(A))
			...
			if (v.getFullYear() != c || v.getMonth() + 1 != q || v.getDate() != k)
				throw "Invalid date";
			return v
		},
		ATOM : "yy-mm-dd",
		...
		_generateHTML : function (a) {
			...
			for (var G = this._getDefaultDate(a), J = "", D = 0; D < i[0]; D++) {
				for (var M = "", E = 0; E < i[1]; E++) {
					...
					if (n == a.selectedYear && g == a.selectedMonth)
		...
		_generateMonthYearHeader : function (a, b, c, e, f, h, i, g) {
			...
			if (h || !m)
				...
				for (var j = 0; j < 12; j++)
					if ((!i || j >= e.getMonth()) && (!n || j <= f.getMonth()))
						o += '<option value="' + j + '"' + (j == b ? ' selected="selected"' : "") + ">" + g[j] + "</option>";
				o += "</select>"
			}
			k || (l += o + (h || !(m && q) ? "\u00a0" : ""));
			a.yearshtml = "";
			if (h || !q)
				l += '<span class="ui-datepicker-year">' + (c + this._yearOffset(a)) + "</span>";
			else {
				...
				for (a.yearshtml += '<select class="ui-datepicker-year" onchange="DP_jQuery_' +
					z + ".datepicker._selectMonthYear('#" + a.id + "', this, 'Y');\" onclick=\"DP_jQuery_" + z + ".datepicker._clickMonthYear('#" + a.id + "');\">"; b <= g; b++)
					a.yearshtml += '<option value="' + b + '"' + (b == c ? ' selected="selected"' : "") + ">" + (b + this._yearOffset(a)) + "</option>";
				a.yearshtml += "</select>";
