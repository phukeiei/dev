(function (h, V, l, m) {
 // alert(h+" / "+V+" / "+ l + "/ "+ m);
	var j = function (e) {
		...
		function s(a, b, c) {
			...
			if (!b.bSortable || -1 == h.inArray("asc", b.asSorting) && -1 == h.inArray("desc", b.asSorting))
				b.sSortingClass = a.oClasses.sSortableNone, b.sSortingClassJUI = "";
				...
				b.sSortingClass = a.oClasses.sSortableDesc, b.sSortingClassJUI = a.oClasses.sSortJUIDescAllowed
		}
		function k(a) {
		...
		function ua(a) {
			...
			for (b = 0, c = e.length; b < c; b++)
				for (d = 0, f = e[b].childNodes.length; d < f; d++)
					g = e[b].childNodes[d], n = g.nodeName.toUpperCase(), ("TD" == n || "TH" == n) && i.push(g);
			for (f = 0, e = a.aoColumns.length; f < e; f++) {
				...
				if (g || u || o || !k)
					for (b = 0, c = a.aoData.length; b < c; b++) {
						...
						if (u)
							s = R(a, b, f), m.innerHTML = s, n.bUseRendered && I(a, b, f, s);
		...
		function va(a) {
			...
			if (0 !== a.nTHead.getElementsByTagName("th").length)
				...
				for (b = 0, d = a.aoColumns.length; b < d; b++)
					c = a.aoColumns[b].nTh, c.innerHTML = a.aoColumns[b].sTitle, c.setAttribute("tabindex", "0"), null !== a.aoColumns[b].sClass && h(c).addClass(a.aoColumns[b].sClass), f.appendChild(c);
			...
			for (var c = a.nTableWrapper, d = a.sDom.split(""), f, g, i, e, u, n, o, k = 0; k < d.length; k++) {
				...
				if ("<" == i) {
					...
					if ("'" == u || '"' == u) {
						...
						for (o = 2; d[k + o] != u; )
							n += d[k + o], o++;
						"H" == n ? n = "fg-toolbar ui-toolbar ui-widget-header ui-corner-tl ui-corner-tr ui-helper-clearfix" : "F" == n && (n = "fg-toolbar ui-toolbar ui-widget-header ui-corner-bl ui-corner-br ui-helper-clearfix");
		...
		function O(a, b, c) {
			...
			for (var b = 0, f = c.length; b < f; b++)
				for (var g = 0, i = c[b].length; g < i; g++)
		...
		function za(a) {
			...
			c = -1 !== c.indexOf("_INPUT_") ? c.replace("_INPUT_", '<input type="text" />') : "" === c ? '<input type="text"  />' : c + ' <input type="text" placeholder="ค้นหา" />',
			...
			c.bind("keyup.DT", function () {
				for (var c = a.aanFeatures.f, d = "" === this.value ? "" : this.value, i = 0, e = c.length; i < e; i++)
					c[i] != h(this).parents("div.dataTables_filter")[0] && h(c[i]._DT_Input).val(d);
				d != b.sSearch && M(a, {
					sSearch : d,
					bRegex : b.bRegex,
					bSmart : b.bSmart,
		...
		function ma(a, b) {
			...
			for (var d = a.__nTmpFilter, f = 0, g = a.aoColumns.length; f < g; f++)
				a.aoColumns[f].bSearchable && (c += la(b[f], a.aoColumns[f].sType) + "  ");
			if (-1 !== c.indexOf("&"))
		...
		function ya(a) {
			...
			h("select", f).bind("change.DT", function () {
				...
				f = a.aanFeatures.l;
				for (c = 0, d = f.length; c < d; c++)
					f[c] != this.parentNode && h("select", f[c]).val(b);
		...
		function Ka(a) {
			...
			if ("" === a.oScroll.sX) {
				...
				if (a.nTable.style.width = q(f - a.oScroll.iBarWidth), h(a.nTable).outerWidth() > f - a.oScroll.iBarWidth)
			...
			} else if (d.style.width = q("100%"), b.parentNode.style.width = q("100%"), null !== a.nTFoot)
		...
		function ba(a) {
			...
			if (f == i.length && 0 === b && d == f && "" === a.oScroll.sX && "" === a.oScroll.sY)
				for (g = 0; g < a.aoColumns.length; g++) {
				...
				for (g = d = 0; g < a.aoColumns.length; g++)
					...
					for (b = 0; b < e; b++)
						if (-1 != d[b].className.indexOf(g + "1"))
							for (c = 0, a = d.length / e; c < a; c++)
								d[e * c + b].className = h.trim(d[e * c + b].className.replace(g + "1", ""));
			...
			if (e[a].bVisible != b) {
				if (b) {
					...
					for (f = 0, g = h.length; f < g; f++)
						null !== h[f].nTr && (n ? h[f].nTr.appendChild(h[f]._anHidden[a]) : h[f].nTr.insertBefore(h[f]._anHidden[a], L(d, f)[o]))
				} else
					for (f = 0, g = h.length; f < g; f++)
		...
		return this.each(function () {
			...
			if ("table" != this.nodeName.toLowerCase())
				...
				if (null !== e.iDeferLoading)
					i.bDeferLoading = !0, a = h.isArray(e.iDeferLoading), i._iRecordsDisplay = a ? e.iDeferLoading[0] : e.iDeferLoading, i._iRecordsTotal = a ? e.iDeferLoading[1] : e.iDeferLoading;
				null !== e.aaData && (g = !0);
	...
	j.defaults = {
		...
		aaSorting : [[0, "asc"]],
		aaSortingFixed : null,
		aLengthMenu : [[10, 25, 50, -1], [10, 25, 50, "ทั้งหมด"]],  // คือส่วนกำหนด ข้อมูลแสเงผล
		aoColumns : null,
		aoColumnDefs : null,
		aoSearchCols : [],
		...
		oLanguage : {
			...
			sEmptyTable : "ไม่มีข้อมูลในฐานข้อมูล",
			sInfo : "มีทั้งหมด _TOTAL_ รายการ",
			sInfoEmpty : "มี 0 รายการ",
			sInfoFiltered : "",
			sInfoPostFix : "",
			sInfoThousands : ",",
	...
	h.extend(j.ext.oPagination, {
		two_button : {
			fnInit : function (e, j, m) {
				...
				if (!e.aanFeatures.p)
					j.id = e.sTableId + "_paginate", k.id = e.sTableId + "_previous", r.id = e.sTableId + "_next", k.setAttribute("aria-controls", e.sTableId), r.setAttribute("aria-controls", e.sTableId)
			},
			fnUpdate : function (e) {
				if (e.aanFeatures.p)
